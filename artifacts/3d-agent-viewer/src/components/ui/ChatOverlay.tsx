...
export function ChatOverlay({ agent, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-lg bg-white sm:rounded-2xl rounded-t-2xl shadow-2xl overflow-hidden flex flex-col"
        style={{
          maxHeight: "min(82dvh, calc(100dvh - env(safe-area-inset-top, 20px) - 8px))",
          minHeight: "clamp(260px, 52dvh, 400px)",
        }}
      >
        {/* Header */}
        <div
        ...
        {/* Input */}
        <div className="px-2.5 py-2.5 border-t border-gray-100 bg-white shrink-0 flex gap-2" style={{ paddingBottom: "max(10px, env(safe-area-inset-bottom, 10px))" }}>
          <input
            ref={inputRef}
            type="text"
            value={input}
