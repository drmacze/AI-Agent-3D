...
export function AgentAvatar({ agent, isSelected, onClick }: Props) {
  ...
  useFrame((state, delta) => {
    ...
    const isMoving = dist > 0.12;

    const status = agent.status;
    const taskLower = (agent.currentTask ?? "").toLowerCase();
    const isStretching = taskLower.includes("explor") || taskLower.includes("wander");

    const isSitting = status === "working" && !isMoving && !isStretching;
    seatProgress.current = THREE.MathUtils.lerp(
      seatProgress.current, isSitting ? 1 : 0, delta * (isSitting ? 2 : 3)
    );
    ...
    if (isMoving) {
      ...
      groupRef.current.rotation.y = THREE.MathUtils.lerp(groupRef.current.rotation.y, Math.PI, delta * 3.5);
    }
