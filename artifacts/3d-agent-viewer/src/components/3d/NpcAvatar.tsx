...
function getActivityEmoji(agent: NpcAgent): { emoji: string; label: string } {
  ...
  if (t.includes("deploy") || t.includes("infra")) return { emoji: "🚀", label: "Deploying" };
  if (t.includes("review") || t.includes("audit")) return { emoji: "🔍", label: "Reviewing" };
  if (t.includes("meet") || t.includes("align")) return { emoji: "📋", label: "Meeting" };
  if (t.includes("report") || t.includes("strat")) return { emoji: "📈", label: "Planning" };
...
export function NpcAvatar({ agent, isSelected = false, onClick }: Props) {
  ...
  useFrame((state, delta) => {
    ...
    const moving = currentPos.current.distanceTo(targetPos) > 0.1;
    ...
    if (leftEyeLidRef.current) leftEyeLidRef.current.scale.y = THREE.MathUtils.lerp(leftEyeLidRef.current.scale.y, bs, delta * 20);
    if (rightEyeLidRef.current) rightEyeLidRef.current.scale.y = THREE.MathUtils.lerp(rightEyeLidRef.current.scale.y, bs, delta * 20);
    ...
    if (moving || agent.status === "moving") {
      ...
      if (headRef.current) headRef.current.rotation.x = 0.07 + Math.sin(t * 0.4) * 0.03;
      ...
      if (headRef.current) {
        headRef.current.rotation.y = Math.sin(t * 0.8 + seed * 0.3) * 0.18;
      ...
      if (bodyRef.current) { bodyRef.current.scale.y = 1 + breathe; bodyRef.current.rotation.z = shift; }
      if (headRef.current) headRef.current.rotation.y = Math.sin(t * 0.25 + seed * 0.4) * 0.09;
  ...
  const activity = getActivityEmoji(agent);

  return (
    <group ref={groupRef} position={[agent.positionX, 0, agent.positionZ]} onClick={e => { e.stopPropagation(); onClick?.(); }}>
      {/* Shadow */}
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.007, 0]} scale={[1, 0.72, 1]}>
        <circleGeometry args={[0.23, 14]} />
        <meshLambertMaterial color="#806040" transparent opacity={0.28} />
      </mesh>
      ...
      <mesh position={[0, 0.645, 0.107]}>
        ...
        <meshLambertMaterial color="#d4a030" />
      </mesh>
