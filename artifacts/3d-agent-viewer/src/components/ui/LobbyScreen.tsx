...
export function LobbyScreen() {
  ...
  if (user) {
    return (
      <div className="absolute inset-0 z-50 flex items-center justify-center"
        ...
        <div className={`transition-all duration-500 ${animate ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-95'}`}
          style={{
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center"
      ...
      <div className={`transition-all duration-600 ${animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
        style={{
          background: 'linear-gradient(135deg,rgba(20,20,50,0.92),rgba(10,10,30,0.95))',
          border: '1px solid rgba(255,255,255,0.12)',
          borderRadius: 24, padding: '40px 36px', width: 400,
          boxShadow: '0 40px 100px rgba(0,0,20,0.8), 0 0 0 1px rgba(255,255,255,0.04) inset'
        }}>

        {/* Logo & title */}
        <div className="text-center mb-8">
          <div style={{ fontSize: 54, marginBottom: 10, filter: 'drop-shadow(0 0 20px rgba(139,92,246,0.6))' }}>🏢</div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: '#ffffff', letterSpacing: '-0.5px', margin: '0 0 4px' }}>DLavie OS</h1>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.06)', borderRadius: 12, marginBottom: 24, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
